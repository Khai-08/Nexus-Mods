...
from utils.api_utils import fetch_trending_mods

class TrendingModsPaginator(View):
    ...
    def get_embed(self):
        ...
        embed = discord.Embed(title=f"Trending Mods", color=self.bot.embed_color)
        desc = ""
...
def setup(bot):
    @bot.tree.command(name="trending-mods", description="Displays the top 10 trending mods for a specified game from Nexus Mods.")
    ...
    async def latest_mods(interaction: discord.Interaction, game: str):
        ...
        try:
            mods = await fetch_trending_mods(game, bot.api_headers)
            if not mods:
                return await bot.error_embed(interaction, f"No trending mods available for {game}.")

            bot.current_game = game
            if len(mods) > 5:
                view = TrendingModsPaginator(bot, interaction, mods)
